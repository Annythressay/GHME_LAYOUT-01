import { ArrowDown, CheckCircle2, RotateCcw, Award, Minus, Info } from 'lucide-react';
import { DURATION, formatTime } from '../game';

export default function Results({ questions, responses, remaining, timedOut, onRestart, headingRef }) {
  ...
  return (
    <section className="mx-auto w-full max-w-240 rounded-3xl border border-line bg-white p-7 shadow-[0_12px_50px_#10365e06] md:p-12">
      ...
      <h1 ref={headingRef} tabIndex={-1} className="question-heading mt-3 text-center text-3xl font-bold text-navy">{timedOut ? '4 phút đã kết thúc' : 'Bạn đã hoàn thành thử thách!'}</h1>
      <p className="mx-auto mt-3 max-w-140 text-center text-sm leading-7 text-muted">{score >= 8 ? 'Bạn đã nắm vững nhiều kiến thức nền tảng. Tiếp tục thực hành để tự tin hơn khi cần giúp đỡ.' : 'Mỗi tình huống là một cơ hội học thêm. Cùng xem lại đáp án và củng cố kiến thức sơ cứu của bạn.'}</p>
      ...
      <div className="flex flex-wrap justify-center gap-3"><button className="button-primary" onClick={onRestart}><RotateCcw size={17} aria-hidden="true" />Thử lại từ đầu</button><a className="button-secondary" href="#review">Xem lại đáp án<ArrowDown size={17} aria-hidden="true" /></a></div>
